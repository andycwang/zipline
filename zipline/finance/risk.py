...
class RiskMetricsBase(object):
    ...
    def choose_treasury(self):
        ...
        if self.end_date in self.treasury_curves:
            ...
            dist = lambda d: abs((d - self.end_date).days) \
                + (.5 if d < self.end_date else 0)  # prev is 'farther'

            # Find leftmost item greater than or equal to search_day
            ...
            if next_day and prev_day:
                search_day = min(sorted([next_day, prev_day], key=dist))
            else:
                search_day = next_day or prev_day

            if dist(search_day) > 3:
                message = "No rate within 3 days of end date = {dt} and \
term = {term}. Check that date doesn't exceed treasury history range."
